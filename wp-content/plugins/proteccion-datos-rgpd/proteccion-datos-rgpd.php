...
include_once( plugin_dir_path( __FILE__ ) . 'aviso-legal.php' );

// Privacy Policy related code loading
include_once( plugin_dir_path( __FILE__ ) . 'politica-privacidad.php' );

// Primera capa del deber de información y casilla de aceptación en formularios
include_once( plugin_dir_path( __FILE__ ) . 'formularios.php' );

// Cookie Policy related code loading
include_once( plugin_dir_path( __FILE__ ) . 'politica-cookies.php' );

// Cookies banner related code loading
include_once( plugin_dir_path( __FILE__ ) . 'banner-cookies.php' );

// Notas a pie de página
include_once( plugin_dir_path( __FILE__ ) . 'pie.php' );

// Lista de variables usadas en tabla options
include_once( plugin_dir_path( __FILE__ ) . 'lista_opciones.php' );
...
function pdrgpd_modulo_jetpack_activo( $modulo ) {
	$activo = false;
	if ( class_exists( 'Jetpack' ) ) {
		//if ( Jetpack::is_module_active( $modulo ) ) {
		//	$activo=true;
		//}
		$modulos_jetpack = get_option( 'jetpack_active_modules' );
		$activo = in_array( $modulo, $modulos_jetpack );
	}
	return $activo;
}
