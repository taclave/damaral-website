...
function pdrgpd_politica_privacidad_finalidad() {
	...
	if ( pdrgpd_existe_suscripcion_jetpack() ) {
		$html .= '<p>';
		$html .= __( 'Blog suscription form', 'proteccion-datos-rgpd' );
		$html .= ': ';
		$html .= __( 'Inform you by e-mail of new posts in the blog.', 'proteccion-datos-rgpd' );
		//$html .= __( 'Informarte por e-mail de nuevas entradas publicadas en el blog.', 'proteccion-datos-rgpd' );
		$html .= "</p>\n";
	}
...
function pdrgpd_politica_privacidad_transferencia() {
	...
	if ( pdrgpd_existe_suscripcion_jetpack() ) {
		$html .= '<p>';
		$html .= __( 'Form to subscribe to the blog by e-mail will be processed by Automattic Inc., with quarters in the US in order to provide that service.', 'proteccion-datos-rgpd');
		//$html .= __( 'El formulario para suscribirse al blog por correo electrónico, será tratado por Automattic Inc., con domicilio en EEUU con la finalidad de prestarle ese servicio.', 'proteccion-datos-rgpd');
		$html .= ' ';
		$html .= __( 'You may review the', 'proteccion-datos-rgpd');
		$html .= ' <a href="';
...
function pdrgpd_politica_privacidad_transferencia_mini( $formulario ) {
	if ( ( pdrgpd_existe_akismet() && 'comentar' == $formulario ) || ( pdrgpd_conf_akismet_formulario_contacto() && 'contacto' == $formulario ) ) {
		...
		//$html = __( 'No data will be transferred to third parties, except legal obligation.', 'proteccion-datos-rgpd' );
		$html = '';
	}
	return $html;
}
...
function pdrgpd_url_privacidad_jetpack () {
	//return 'https://automattic.com/es/privacy/';
...
function pdrgpd_url_traducida ( $url_original, $idioma_origen ) {
	...
	if ( $locale == $idioma_origen ){
		...
		$uri_localizada .= '&u=' . urlencode ( $url_original );
	}
