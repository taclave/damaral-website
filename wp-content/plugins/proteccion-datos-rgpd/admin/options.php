...
//add_action ('admin_init', 'pdrgpd_checkbox_crear_paginas');
//function pdrgpd_checkbox_crear_paginas() {
//	global $_POST;
//	if($_POST['checkbox'] == 1) 	{
...
function pdrgpd_settings_init() {
	...
	// Owner and liable data
	// Datos del titular y responsable
	...
	if( isset( $_POST['pdrgpd_crear_paginas_legales'] ) ) {
		// Aquí sí lo detecta.
		die('pdrgpd_crear_paginas_legales');
		// pdrgpd_faltan_paginas_legales() 
	}
	*/
	...
	add_settings_section (
		'pdrgpd_seccion_banner',
		__( 'Cookies banner', 'proteccion-datos-rgpd' ),
		'pdrgpd_seccion_banner_callback',
		'proteccion-datos-rgpd-ajustes'
	);

	add_settings_field (
		'pdrgpd_mostrar_banner_cookies',
		__( 'Show cookies banner', 'proteccion-datos-rgpd' ),
		'pdrgpd_mostrar_banner_cookies_callback',
		'proteccion-datos-rgpd-ajustes',
		'pdrgpd_seccion_banner'
	);
	...
	add_settings_field (
		...
		//__( 'Enlace al aviso legal', 'proteccion-datos-rgpd' ),
	...
	add_settings_field (
		...
		//__( 'Líneas diferentes para enlaces y copyright', 'proteccion-datos-rgpd' ),
...
function pdrgpd_seccion_titular_callback() { 
	echo __( 'General data required to fulfill legal notice according 34/2002 law, July 11, about Services of the Information Society and E-commerce (LSSICE) and others.<br />Fill appropriate fields.', 'proteccion-datos-rgpd' );
}
...
function pdrgpd_existencia_suscripcion_jetpack_callback() {
	if ( class_exists( 'Jetpack' ) ){
		...
		//	echo 'No están habilitadas los widgets en Jetpack<br />';
		//}
	}
	else {
...
// Banner de cookies
function pdrgpd_seccion_banner_callback() { 
	echo __( 'Cookies banner settings.', 'proteccion-datos-rgpd' );
	echo '<br />';
}

function pdrgpd_mostrar_banner_cookies_callback() {
	echo "<input type='checkbox' name='pdrgpd_mostrar_banner_cookies' ";
	checked( get_option( 'pdrgpd_mostrar_banner_cookies' ) , 1 );
	echo " value='1'> ";
	echo __( 'Show banner prompting for permission to load cookies' , 'proteccion-datos-rgpd' );
}
...
function pdrgpd_seccion_pie_callback() { 
	...
	echo '<br />';
	echo __( 'Compatible themes' , 'proteccion-datos-rgpd' ) . ': Twenty Twelve, Twenty Thirteen, Twenty Fourteen, Twenty Fifteen, Twenty Sixteen, Twenty Seventeen, Twenty Nineteen, Twenty Twenty, Storefront, Flash, ColorMag, eStore, Spacious, Cenote ' . __( 'and' , 'proteccion-datos-rgpd' ) . ' Envo Shop.<br />';
	//echo 'Tema padre actual: <b>' . esc_html( tema_padre() ) . '</b>.<br />';
...
function pdrgpd_slug_pagina( $url ) {
	...
	//if ( $url ){
		...
		//else{
		//	trigger_error ( "Intentando conseguir slug de $url", E_USER_WARNING );
		//}
	//}
	return $slug;
}
...
function pdrgpd_faltan_paginas_legales() {
	...
	if( $slug && !pdrgpd_existe_pagina( $slug ) ) { $faltan = true; }
	...
	if( $slug ){
		if ( pdrgpd_existe_pagina( $slug ) ) {
			...
			if( wp_insert_post( $nueva_pagina , true ) ) {
				...
				//$message = sprintf( __( 'Creada la página %s.' , 'proteccion-datos-rgpd' ) , pdrgpd_enlace_pagina_wp( $url ) );
