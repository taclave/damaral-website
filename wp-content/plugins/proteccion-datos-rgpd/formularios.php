...
function pdrgpd_parrafo_primera_capa( $finalidad , $transferencia, $responsable, $url_privacidad, $gestion ) {
	...
	if( $gestion ){
		$html .= __( 'You have the right to access, rectify and delete the data, as well as other rights' , 'proteccion-datos-rgpd' ) . ' ' . __( 'through' , 'proteccion-datos-rgpd' ) . ' <a href="' . esc_html( $gestion ) . '" target="_blank" rel="noopener noreferrer">' . __( 'here' , 'proteccion-datos-rgpd' ) . '</a>. ';
		...
		$html .= __( 'You have the right to access, rectify and delete the data, as well as other rights, as explained in the' , 'proteccion-datos-rgpd' ) . ' <a href="[pdrgpd-uri-privacidad]" target="_blank" rel="noopener noreferrer">' . __( 'privacy policy' , 'proteccion-datos-rgpd' ) . '</a>.';
	}
	$html .= "</p>\n";
	return $html;
}

// __( '' , 'proteccion-datos-rgpd' )
...
function pdrgpd_tabla_primera_capa( $finalidad , $transferencia, $responsable, $url_privacidad, $gestion ) {
	...
	if( $responsable ){
		$html .= pdrgpd_fila_tabla_primera_capa( __( 'Responsible' , 'proteccion-datos-rgpd' )                  , esc_html( __( $responsable ) ) . ' '. pdrgpd_enlace_mas_info( $url_privacidad, 'responsable' ) );
	}
	else{
	...
	if( $transferencia ){
		$html .= pdrgpd_fila_tabla_primera_capa( __( 'Recipients' , 'proteccion-datos-rgpd' )            , esc_html( __( $transferencia ) ) . ' '. pdrgpd_enlace_mas_info( $url_privacidad, 'transferemcia' ) );
	}
	else{
	...
	if( $gestion ){
		$html .= pdrgpd_fila_tabla_primera_capa( __( 'Rights' , 'proteccion-datos-rgpd' )                     , __( 'Access, rectify and delete data, as well as some other rights' , 'proteccion-datos-rgpd' ) . ' ' . __( 'through' , 'proteccion-datos-rgpd' ) . ' '. pdrgpd_enlace_mas_info( esc_html( $gestion ), '' ) );
	}
	else{
		$html .= pdrgpd_fila_tabla_primera_capa( __( 'Rights' , 'proteccion-datos-rgpd' )                     , __( 'Access, rectify and delete data, as well as some other rights' , 'proteccion-datos-rgpd' ) . '. '. pdrgpd_enlace_mas_info( $url_privacidad, 'derechos' ) );
	}
...
function pdrgpd_enlace_mas_info( $url_privacidad, $id ) {
	...
	if( $url_privacidad ){
		...
		# El id solo actúa sobr ela URL propia
		if ( $id ){
			$html .= "#$id";
		}
	}
...
function pdrgpd_aviso_tras_form_comentar($args) {
	//$args['comment_notes_after'] = __( 'Texto para RGPD', 'proteccion-datos-rgpd' );
...
function pdrgpd_requiere_aceptar_privacidad( $fields ) {
	if ( ( ! isset( $_POST['pdrgpd_acepto_politica_privacidad'] ) ) || $_POST['pdrgpd_acepto_politica_privacidad'] != 'acepto' ) {
		wp_die('<p><strong>ERROR</strong>: ' . __( 'You must accept the privacy policy to send comments', 'proteccion-datos-rgpd' ) . '.</p>' . "\n" . '<p><a href=\'javascript:history.back()\'>&laquo; ' . __( 'Return', 'proteccion-datos-rgpd' ) . '</a></p>');
	}
	return $fields;
}
...
function pdrgpd_aceptacion_privacidad_grabar ( $post_id ) {
	$acepta_privacidad = $_POST['pdrgpd_acepto_politica_privacidad'];
...
function pdrgpd_aceptacion_privacidad_mostrar() {
	echo get_comment_text(), '<br><br><strong>Política privacidad: ', get_comment_meta(get_comment_ID(), 'pdrgpd_acepto_politica_privacidad',1), '<strong>';
}
...
function pdrgpd_existe_akismet() {
	...
	// if ( get_option( 'wordpress_api_key'                   ) ) { $existe_akismet = true }
...
// Un valor por defecto configurado en español es traducido a francés o inglés para mostrarlo en la primera capa cuandoel sitio es multiidioma
