...
function pdrgpd_ob_pie_callback( $buffer ) {
	...
	switch ( tema_padre() ) {
		case "Flash":				// Tema Flash de ThemeGrill
			$buffer = preg_replace( '/<span class="copyright-text">(.*?)<\/span>/s' , '<span class="copyright-text">' . $pie_completo . '</span>' , $buffer );
			break;
		case "ColorMag":			// Tema ColorMag de ThemeGrill
			$buffer = preg_replace( '/<span class="copyright">(.*?)<\/span>/s' , '<span class="copyright">' . $pie_completo . '</span>' , $buffer );
			break;
		case "eStore":				// Tema eStore de ThemeGrill
			$buffer = preg_replace( '/<div class="copy-right">(.*?)<\/div>/s' , '<div class="copy-right">' . $pie_completo . '</div>' , $buffer );
			break;
		case "Spacious":			// Tema Spacious de ThemeGrill
			$buffer = preg_replace( '/<div class="copyright">(.*?)<\/div>/s' , '<div class="copyright">' . $pie_completo . '</div>' , $buffer );
			break;
		case "Industro":			// Tema Industro de OceanThemes
			$buffer = preg_replace( '/<div class="footer-copyright">(.*?)<\/div>/s' , '<div class="footer-copyright">' . $pie_completo . '</div>' , $buffer );
			break;
		case "Envo Shop":			// Tema Envo Shop de EnvoThemes
			$buffer = preg_replace( '/<div class="footer-credits-text text-center">(.*?)<\/div>/s' , '<div class="footer-credits-text text-center">' . $pie_completo . '</div>' , $buffer );
			break;
		case "Twenty Twenty":		// Tema Twenty Twenty de WordPress
			$buffer = preg_replace( '/<div class="footer-credits">(.*?)<\/div>/s' , '<div class="copyright">' . $pie_completo . '</div>' , $buffer );
			break;
		case "Twenty Twenty-One":	// Tema Twenty Twenty-One de WordPress
			$buffer = preg_replace( '/<div class="powered-by">(.*?)<\/div>/s' , '<div class="powered-by">' . $pie_completo . '</div>' , $buffer );
			break;
		default:					// Temas originales de WordPress compatibles: Twenty Twelve, Twenty Thirteen, Twenty Fourteen, Twenty Fifteen, Twenty Sixteen, Twenty Seventeen y Twenty Nineteen de WordPress.
			...
			$buffer = preg_replace( '/<div class="site-info">(.*?)<\/div>/s' , '<div class="site-info">' . $pie_completo . '</div>' , $buffer );
			break;
	}
...
function pdrgpd_pie_linea_copyright() { 
	if( pdrgpd_pie_linea_copyright_utilizada() ){
		...
		$html .= ' ';
		if( pdrgpd_nif_o_cif( pdrgpd_conf_nif() ) == 'CIF' ) {
			// Empresas
		...
		if( $pie_enlace_privacidad_utilizada && $pie_enlace_cookies_utilizada ) {
			...
			if( $pie_enlace_aviso_utilizado ) {
				...
				if( pdrgpd_conf_uri_privacidad() == pdrgpd_conf_uri_cookies() ) {
					...
					$html .= sprintf( __( '<a href="%s">privacy</a> and <a href="%s">cookies</a> policies' , 'proteccion-datos-rgpd' ) , esc_attr( pdrgpd_conf_uri_privacidad() ) , esc_attr( pdrgpd_conf_uri_cookies() ) );
				}
			}
			else{
				if( pdrgpd_conf_uri_privacidad() == pdrgpd_conf_uri_cookies() ) {
